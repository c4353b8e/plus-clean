namespace Plus.Communication.Packets.Incoming.Rooms.Engine
{
    ...
    internal class PlaceObjectEvent : IPacketEvent
    {
        public void Parse(GameClient session, ClientPacket packet)
        {
            ...
            if (room.GetRoomItemHandler().GetWallAndFloor.Count() > Convert.ToInt32(Program.SettingsManager.TryGetValue("room.item.placement_limit")))
            {
                session.SendNotification("You cannot have more than " + Convert.ToInt32(Program.SettingsManager.TryGetValue("room.item.placement_limit")) + " items in a room!");
                return;
            }
            ...
            if (!item.IsWallItem)
            {
                ...
                if (room.GetRoomItemHandler().SetFloorItem(session, roomItem, x, y, rotation, true, false, true))
                {
                    ...
                    {
                        Program.GameContext.GetAchievementManager().ProgressAchievement(session, "ACH_RoomDecoFurniCount", 1);
                    }
