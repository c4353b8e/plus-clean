namespace Plus.Communication.Packets
{
    ...
    {
        private static readonly ILogger Logger = new Logger<PacketManager>();
        ...
        public void TryExecutePacket(GameClient session, ClientPacket packet)
        {
            ...
            if (System.Diagnostics.Debugger.IsAttached)
            {
                ...
                {
                    Logger.Debug("Handled Packet: [" + packet.Id + "] " + _packetNames[packet.Id]);
                }
                else
                {
                    Logger.Debug("Handled Packet: [" + packet.Id + "] UnnamedPacketEvent");
                }
            }
