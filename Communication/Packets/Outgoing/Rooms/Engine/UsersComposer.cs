    ...
    internal class UsersComposer : ServerPacket
    {
        ...
        private void WriteUser(RoomUser User)
        {
            if (!User.IsPet && !User.IsBot)
            {
                ...
                if (Habbo != null)
                {
                    if (Habbo.GetStats() != null)
                    {
                        ...
                        {
                            if (!Program.GameContext.GetGroupManager().TryGetGroup(Habbo.GetStats().FavouriteGroupId, out Group))
                            {
                                Group = null;
                            }
                        }
            ...
            else if (User.IsBot)
            {
                ...
                WriteString(HabboFactory.GetUsernameById(User.BotData.OwnerId)); // Owner name
