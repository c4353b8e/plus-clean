namespace Plus.HabboHotel.Rooms
{
    ...
    {
        private static readonly ILogger Logger = new Logger<RoomManager>();
            ...
            using (var dbClient = Program.DatabaseManager.GetQueryReactor())
            {
                ...
                foreach (DataRow row in data.Rows)
                {
                    ...
                    _roomModels.Add(model, new RoomModel(model, Convert.ToInt32(row["door_x"]), Convert.ToInt32(row["door_y"]), (double)row["door_z"], Convert.ToInt32(row["door_dir"]),
                        Convert.ToString(row["heightmap"]), row["club_only"].ToString() == "1", Convert.ToInt32(row["wall_height"]), false));
                }
            }
        }
            ...
            using (var dbClient = Program.DatabaseManager.GetQueryReactor())
            {
                ...
                if (!_roomModels.ContainsKey(model))
                {
                    _roomModels.Add(model, new RoomModel(model, Convert.ToInt32(row["door_x"]), Convert.ToInt32(row["door_y"]), Convert.ToDouble(row["door_z"]), Convert.ToInt32(row["door_dir"]),
                      Convert.ToString(row["heightmap"]), row["club_only"].ToString() == "1", Convert.ToInt32(row["wall_height"]), true));
                }
        ...
        {
            if (_rooms.TryGetValue(roomId, out var inst))
            {
                if (!inst.Unloaded)
                {
                    room = inst;
        ...
        public void Dispose()
        {
            ...
            foreach (var room in _rooms.Values.ToList())
            {
                ...
                Console.Clear();
                Logger.Trace("<<- SERVER SHUTDOWN ->> ROOM ITEM SAVE: " + string.Format("{0:0.##}", (double)i / length * 100) + "%");
                i++;
            }
            Logger.Trace("Done disposing rooms!");
        }
    }
}