namespace Plus.Game.Rooms
{
    ...
    public class RoomItemHandling
    {
            ...
            foreach (var Item in Items.ToList())
            {
                ...
                if (Item.IsFloorItem)
                {
                    if (!_room.GetGameMap().ValidTile(Item.GetX, Item.GetY))
                    {
                        ...
                        var Client = Program.GameContext.PlayerController.GetClientByUserId(Item.UserID);
                        if (Client != null)
                        {
        ...
        public void RemoveFurniture(Player session, int id)
        {
            var Item = GetItem(id);
            if (Item == null)
            {
        ...
        public ServerPacket UpdateItemOnRoller(Item pItem, Point NextCoord, int pRolledID, double NextZ)
        {
            ...
            mMessage.WriteString(StringUtilities.GetString(pItem.GetZ));
            mMessage.WriteString(StringUtilities.GetString(NextZ));
        ...
        public ServerPacket UpdateUserOnRoller(RoomUser pUser, Point pNextCoord, int pRollerID, double NextZ)
        {
            ...
            mMessage.WriteString(StringUtilities.GetString(pUser.Z));
            mMessage.WriteString(StringUtilities.GetString(NextZ));
        ...
        public bool SetWallItem(Player session, Item item)
        {
        ...
        public List<Item> RemoveItems(Player session)
        {
