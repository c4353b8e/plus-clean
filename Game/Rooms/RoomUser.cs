namespace Plus.Game.Rooms
{
    ...
    public class RoomUser
    {
        ...
        public void OnChat(int Colour, string Message, bool Shout)
        {
            ...
            if (Shout)
            {
                Packet = new ShoutComposer(VirtualId, Message, Program.GameContext.GetChatManager().GetEmotions().GetEmotionsForText(Message), Colour);
            }
            else
            {
                Packet = new ChatComposer(VirtualId, Message, Program.GameContext.GetChatManager().GetEmotions().GetEmotionsForText(Message), Colour);
            }
        ...
        public Player GetClient()
        {
            ...
            return mClient ?? (mClient = Program.GameContext.PlayerController.GetClientByUserId(HabboId));
        }
        ...
        private Room GetRoom()
        {
            if (mRoom == null)
            {
                if (Program.GameContext.GetRoomManager().TryGetRoom(RoomId, out mRoom))
                {
                    return mRoom;
                }
            }
