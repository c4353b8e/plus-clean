    ...
    public class Pet
    {
        ...
        public Room Room
        {
            get
            {
                ...
                if (Program.GameContext.GetRoomManager().TryGetRoom(RoomId, out var _room))
                {
                    return _room;
                }
        ...
        public int Age => Convert.ToInt32(Math.Floor((UnixUtilities.GetNow() - CreationStamp) / 86400));

        public string Look => Type + " " + Race + " " + Color + " " + GnomeClothing;

        public string OwnerName => Program.GameContext.PlayerController.GetNameById(OwnerId);
    }
}