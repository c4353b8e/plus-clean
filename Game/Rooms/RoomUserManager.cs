namespace Plus.Game.Rooms
{
    ...
    public class RoomUserManager
    {
        ...
        public bool AddAvatarToRoom(Player session)
        {
            if (_room == null)
            {
                return false;
        ...
        public void RemoveUserFromRoom(Player session, bool nofityUser, bool notifyKick = false)
        {
            try
            {
                if (_room == null)
                ...
                if (User != null)
                {
                    ...
                    {
                        dbClient.RunQuery("UPDATE user_roomvisits SET exit_timestamp = '" + UnixUtilities.GetNow() + "' WHERE room_id = '" + _room.RoomId + "' AND user_id = '" + session.GetHabbo().Id + "' ORDER BY exit_timestamp DESC LIMIT 1");
        ...
        public void OnCycle()
        {
            ...
            try
            {
                        ...
                        else
                        {
                            ...
                            if (_room.GetGameMap().IsValidStep2(User, new Vector2D(User.X, User.Y), new Vector2D(nextX, nextY), User.GoalX == nextX && User.GoalY == nextY, User.AllowOverride))
                            {
                                ...
                                if (!User.IsBot && User.RidingHorse && User.IsPet == false)
                                {
                                    ...
                                    if (Horse != null)
                                    {
                                        Horse.SetStatus("mv", nextX + "," + nextY + "," + StringUtilities.GetString(nextZ));
                                    }

                                    User.SetStatus("mv", +nextX + "," + nextY + "," + StringUtilities.GetString(nextZ + 1));
                                ...
                                {
                                    User.SetStatus("mv", nextX + "," + nextY + "," + StringUtilities.GetString(nextZ));
                                }
                ...
                {
                    var client = Program.GameContext.PlayerController.GetClientByUserId(toRemove.HabboId);
                    if (client != null)
                    {
        ...
        public void UpdateUserStatus(RoomUser User, bool cyclegameitems)
        {
            ...
            try
            {
                ...
                if (UnixUtilities.GetNow() > UnixUtilities.GetNow() + User.SignTime)
                {
                    if (User.Statusses.ContainsKey("sign"))
                    {
                ...
                foreach (var Item in ItemsOnSquare.ToList())
                {
                    ...
                    if (Item.GetBaseItem().IsSeat)
                    {
                        if (!User.Statusses.ContainsKey("sit"))
                        {
                            if (!User.Statusses.ContainsKey("sit"))
                            {
                                User.Statusses.Add("sit", StringUtilities.GetString(Item.GetBaseItem().Height));
                            }
                        }
                        ...
                        case InteractionType.TENT_SMALL:
                            {
                                if (!User.Statusses.ContainsKey("lay"))
                                {
                                    User.Statusses.Add("lay", StringUtilities.GetString(Item.GetBaseItem().Height) + " null");
                                }
