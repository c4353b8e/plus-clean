        ...
        public static List<RoomData> GetRoomsDataByOwnerSortByName(int ownerId)
        {
            ...
            using (var dbClient = Program.DatabaseManager.GetQueryReactor())
            {
                ...
                if (rooms != null)
                {
                    ...
                    {
                        if (Program.GameContext.GetRoomManager().TryGetRoom(Convert.ToInt32(row["id"]), out var room))
                        {
                            data.Add(room);
                        }
                        else
                        {
                            if (!Program.GameContext.GetRoomManager().TryGetModel(Convert.ToString(row["model_name"]), out var model))
                            {
                                continue;
                            }
