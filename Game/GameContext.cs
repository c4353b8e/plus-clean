    ...
    public class GameContext : IDisposable
    {
        ...
        private void GameCycle()
        {
            while (_cycleActive)
            {
                ...
                Program.GameContext.PlayerController.OnCycle();
        ...
        public void Dispose()
        {
            PlayerController.SendPacket(new BroadcastMessageAlertComposer(Program.LanguageManager.TryGetValue("server.shutdown.message")));
            StopGameLoop();

            PacketManager.UnregisterAll();
            PacketManager.WaitForAllToComplete();

            PlayerController.CloseAll();
