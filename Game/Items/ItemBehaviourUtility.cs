        ...
        public static void GenerateExtradata(Item item, ServerPacket packet)
        {
            switch (item.GetBaseItem().InteractionType)
            {
                ...
                case InteractionType.GUILD_FORUM:
                    Group Group = null;
                    if (!Program.GameContext.GetGroupManager().TryGetGroup(item.GroupId, out Group))
                    {
                        packet.WriteInteger(1);
                        packet.WriteInteger(0);
                        packet.WriteString(item.ExtraData);
                    ...
                    else
                    {
                        ...
                        packet.WriteString(Program.GameContext.GetGroupManager().GetColourCode(Group.Colour1, true));
                        packet.WriteString(Program.GameContext.GetGroupManager().GetColourCode(Group.Colour2, false));
                    }
                    break;
                ...
                case InteractionType.GIFT:
                    {
                        ...
                        else
                        {
                            ...
                            var Purchaser = Program.GameContext.GetCacheManager().GenerateUser(Convert.ToInt32(ExtraData[2]));
                            if (Purchaser == null)
                            {
                                packet.WriteInteger(0);
                                packet.WriteInteger(0);
                ...
                case InteractionType.TELEVISION:
                    ...
                    //Message.WriteString("http://img.youtube.com/vi/" + Program.Game.GetTelevisionManager().TelevisionList.OrderBy(x => Guid.NewGuid()).FirstOrDefault().YouTubeId + "/3.jpg");
                    packet.WriteString("");
                    break;
