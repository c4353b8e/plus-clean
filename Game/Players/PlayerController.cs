namespace Plus.Game.Players
{
    ...
    {
        private static readonly ILogger Logger = new Logger<PlayerController>();

        private readonly ConcurrentDictionary<int, Player> _clients;
        private readonly ConcurrentDictionary<int, Player> _userIdRegister;
        private readonly ConcurrentDictionary<string, Player> _usernameRegister;
        ...
        {
            _clients = new ConcurrentDictionary<int, Player>();
            _userIdRegister = new ConcurrentDictionary<int, Player>();
            _usernameRegister = new ConcurrentDictionary<string, Player>();
        ...
        public Player GetClientByUserId(int userId)
        {
            return _userIdRegister.ContainsKey(userId) ? _userIdRegister[userId] : null;
        }

        public Player GetClientByUsername(string username)
        {
            return _usernameRegister.ContainsKey(username.ToLower()) ? _usernameRegister[username.ToLower()] : null;
        }

        public bool TryGetClient(int clientId, out Player client)
        {
            return _clients.TryGetValue(clientId, out client);
        }

        public bool UpdateClientUsername(Player client, string oldUsername, string newUsername)
        {
        ...
        public IEnumerable<Player> GetClientsById(Dictionary<int, MessengerBuddy>.KeyCollection users)
        {
        ...
        {
            var client = new Player(clientId, connection);
        ...
        public void RegisterClient(Player client, int userId, string username)
        {
            ...
            if (_timedOutConnections.Count > 0)
            {
                lock (_timedOutConnections.SyncRoot)
                {
                    while (_timedOutConnections.Count > 0)
                    {
                        ...
                        if (_timedOutConnections.Count > 0)
                        {
                            client = (Player)_timedOutConnections.Dequeue();
                        }
        ...
        public ICollection<Player> GetClients => _clients.Values;
    }
}