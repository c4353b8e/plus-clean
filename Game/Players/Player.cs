namespace Plus.Game.Players
{
    ...
    public class Player
    {
        private static readonly ILogger Logger = new Logger<Player>();
        ...
        public Player(int clientId, ConnectionInformation connection)
        {
        ...
        private void Parser_onNewPacket(ClientPacket message)
        {
            try
            {
                Program.GameContext.PacketManager.TryExecutePacket(this, message);
            }
            catch (Exception e)
            {
                Logger.Error(e);
        ...
        public bool TryAuthenticate(string authTicket)
        {
            try
            {
                ...
                Program.GameContext.PlayerController.RegisterClient(this, userData.UserId, userData.user.Username);
