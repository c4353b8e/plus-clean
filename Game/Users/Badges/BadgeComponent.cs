namespace Plus.Game.Users.Badges
{
    ...
    public class BadgeComponent
    {
        ...
        public BadgeComponent(Habbo Player, UserData.UserData data)
        {
            ...
            foreach (var badge in data.badges)
            {
                ...
                if (!Program.GameContext.GetBadgeManager().TryGetBadge(badge.Code, out BadgeDefinition) || BadgeDefinition.RequiredRight.Length > 0 && !Player.GetPermissions().HasRight(BadgeDefinition.RequiredRight))
                {
                    continue;
                }
        ...
        public void GiveBadge(string code, bool inDatabase, Player session)
        {
            if (HasBadge(code))
            {
                return;
            ...
            if (!Program.GameContext.GetBadgeManager().TryGetBadge(code.ToUpper(), out badge) || badge.RequiredRight.Length > 0 && !session.GetHabbo().GetPermissions().HasRight(badge.RequiredRight))
            {
                return;
            }
